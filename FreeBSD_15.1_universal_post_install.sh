...
base_config() {
    ...
    # Ajout de ca_root_nss pour régler les problèmes de certificats (KDE Themes, wget, etc.)
...
hardware_config() {
    bsddialog --infobox "Installing Xorg, Wayland, Audio, and Peripherals..." 5 50
    pkg install -y pulseaudio pipewire wireplumber audio/freedesktop-sound-theme \
                   xorg wayland xwayland egl-wayland qt5-wayland qt6-wayland \
                   dbus avahi signal-cli seatd sddm \
...
while true; do
    MAIN_CHOICE=$(bsddialog --backtitle "$BACKTITLE" --title "$TITLE" \
        ...
        "3" "Hardware Base (Audio, Xorg, Wayland, CUPS)" \
