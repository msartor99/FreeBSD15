...
# Version: 6.3 - FIX Syntax Error (Network Menu) | Multi-Desktop | Idempotent
# ==============================================================================
...
case "$CHOIX_IF" in
    WIFI) W_DEV=$(sysctl -n net.wlan.devices | awk '{print $1}') ; sysrc wlans_"$W_DEV"="wlan0" ; sysrc ifconfig_wlan0="WPA SYNCDHCP" ;;
...
# 11. FINALISATION
if ! grep -E "^hosts:.*mdns" /etc/nsswitch.conf >/dev/null; then sed -i '' 's/^hosts:.*/& mdns/' /etc/nsswitch.conf; fi
