...
BACKTITLE="Workstation Configuration by Gemini (Pure X11 Edition)"
...
Do you accept these conditions to continue?"
    ...
    if ! bsddialog --backtitle "$BACKTITLE" --title "Warning & Conditions" --yesno "$msg" 14 75; then
        clear
...
base_config() {
    ...
    # Ajout de ca_root_nss pour régler les problèmes de certificats
    pkg install -y doas unzip libzip wget git linux-rl9 htop neofetch python3 bashtop ImageMagick7 smartmontools ca_root_nss
    certctl rehash
...
EOF
    ...
    USER_NAME=$(bsddialog --inputbox "Local Configuration:\nEnter main user name:" 9 50 "administrateur" 3>&1 1>&2 2>&3)
    if [ -n "$USER_NAME" ]; then
        export USER_NAME
...
hardware_config() {
    bsddialog --infobox "Installing Xorg, Audio, and Peripherals (Pure X11)..." 5 50
    pkg install -y pulseaudio pipewire wireplumber audio/freedesktop-sound-theme \
                   xorg dbus avahi signal-cli sddm \
    ...
    sysrc dbus_enable=YES avahi_enable=YES sddm_enable=YES sddm_lang="ch_FR"
...
nvidia_config() {
    ...
    nvidia-xconfig
    bsddialog --msgbox "Nvidia drivers configured successfully for X11!" 6 60
}
...
kamila_splash() {
    bsddialog --infobox "Configuration du Boot Splash Kamila (Redimensionnement dynamique)..." 5 70
    mkdir -p /boot/images
    ...
    if fetch -q -o "${SPLASH_PNG}" "${SPLASH_URL}"; then
        ...
        # Injection des variables dans loader.conf
        ...
        # Nettoyage du fichier PNG original
        rm -f "${SPLASH_PNG}"
        bsddialog --msgbox "Le Splash Screen a été redimensionné en ${FB_W}x${FB_H} et configuré avec succès !" 8 60
    else
        bsddialog --msgbox "Erreur lors du téléchargement de l'image de Splash Screen." 6 60
    fi
}
